...
u5 = create 2, '佐藤　直一'    , 'user4', 'user4', 'user4@demo.joruri.org'
u6 = create 2, '鈴木　裕介'    , 'user5', 'user5', 'user5@demo.joruri.org'
u7 = create 2, '高橋　和寿'    , 'user6', 'user6', 'user6@demo.joruri.org'
u8 = create 2, '田中　彩子'    , 'user7', 'user7', 'user7@demo.joruri.org'
u9 = create 2, '渡辺　真由子'  , 'user8', 'user8', 'user8@demo.joruri.org'
u10 = create 2, '伊藤　勝'     , 'user9', 'user9', 'user9@demo.joruri.org'
...
Sys::UsersGroup.create :user_id => 5, :group_id => g.id + 1
Sys::UsersGroup.create :user_id => 6, :group_id => g.id + 1
Sys::UsersGroup.create :user_id => 7, :group_id => g.id + 1

Sys::UsersGroup.create :user_id => 8, :group_id => g.id + 2
Sys::UsersGroup.create :user_id => 9, :group_id => g.id + 2
Sys::UsersGroup.create :user_id => 10, :group_id => g.id + 2
...
require 'json'
management_path = "#{Rails.root.to_s}/db/managements.json"
management_data = JSON.parse(File.read(management_path))

config_data = Sns::Management.first || Sns::Management.new
management_data.each do |key, value|
  next if key == "_id" || key == "updated_at" || key == "created_at"
  config_data[key.to_sym] = value
end
config_data.save

profile_datas_path = "#{Rails.root.to_s}/db/profiles.json"
profile_json = JSON.parse(File.read(profile_datas_path))
profile_json.each do |profile_data|
  prof = Sns::Profile.where(:account=>profile_data["account"]).first || Sns::Profile.new
  profile_data.each do |key, value|
    next if key == "_id" || key == "updated_at" || key == "created_at"
    if key == "office"
      office_params = {}
      value.each do |k, v|
        next if k == "_id" || k == "updated_at" || k == "created_at"
        office_params[k.to_sym] = v
      end
      prof_office = prof.office
      if prof_office.blank?
        prof.office = Sns::Office.new(office_params)
      else
        prof_office.update_attributes(office_params)
        prof_office.save
      end
    else
      prof[key.to_sym] = value
    end
  end
  prof.save
end
