...
class Sys::Group < ActiveRecord::Base
  ...
  has_many :enabled_children  , :foreign_key => :parent_id, :class_name => 'Sys::Group',
    ...
    #:order => :code, :dependent => :destroy
    :order => :sort_no, :dependent => :destroy

  has_many :code_enabled_children  , :foreign_key => :parent_id, :class_name => 'Sys::Group',
    :conditions => {:state => 'enabled'},
    :order => :code, :dependent => :destroy
    #:order => :sort_no, :dependent => :destroy
  ...
  def users_having_email(order = "id")
    if Application.config(:show_only_ldap_user) == 1
