...
class Sys::User < ActiveRecord::Base
  ...
  def profile
    ret = Sns::Profile.where(:user_id=>self.id).first
    ret = Sns::Profile.create(:user_id=>self.id , :name=>self.name, :account=>self.account) if ret.blank?
    return ret
  end

  def profile_destroy
    prof = Sns::Profile.find( :first, :conditions => {:account => self.account} )
    unless prof.blank?
      prof.destroy
    end
  end
