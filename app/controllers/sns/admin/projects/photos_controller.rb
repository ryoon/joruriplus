# encoding: utf-8
class Sns::Admin::Projects::PhotosController < Sys::Controller::Admin::Base
  ...
  def index
    @items = Sns::Photo.where(:project_id => @project.id, :is_project=>1).desc(:created_at).paginate(:page => params[:page], :per_page => 20)
  end
  ...
  def new
    @item = Sns::Photo.new({:privacy=>"public", :created_user_id=>Core.profile.id, :is_project=>1, :project_id=>@project.id})
  end
  ...
  def destroy
    @item = Sns::Photo.find(:first, :conditions=>{:_id=>params[:id]})
    @item.destroy
    return_uri = redirect_to sns_project_photos_path(@project_code)
