# encoding: utf-8
class Sns::Admin::CommentsController < Sys::Controller::Admin::Base
  ...
  def new
    @item = Sns::Comment.new
    @comments = @post.comments
  end

  def create
    @item = Sns::Comment.new(params[:comment])
    @item.created_user_name = Core.user.name
    @item.created_group_name = Core.user_group.name
    @item.created_group_code = Core.user_group.code
    @item.created_group_id = Core.user_group.id
    @post.comments << @item
    @post.sequence_number if @post.privacy!="project_activity"
    @post.updated_at = Time.now
    @post.save
    feed_post(@post,params[:comment],@item.id)
    @comments = @post.comments
