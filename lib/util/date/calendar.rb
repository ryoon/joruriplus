...
class Util::Date::Calendar
  ...
  def initialize(year = nil, month = nil, options={})
    ...
    sw.times do |i|
      d = pe - (sw - i) + 1
      day = {:year => py, :month => pm, :day => d, :class => 'prevMonth day', :range=>'prevMonth'}
      @days << day
    end
    ed.times do |i|
      day = {:year => cy, :month => cm, :day => (i + 1), :class => 'day', :range=>nil}
      @days << day
    end
    (6 - ew).times do |i|
      day = {:year => ny, :month => nm, :day => (i + 1), :class => 'nextMonth day', :range=>'nextMonth'}
      @days << day
    end
    if options[:form_w]=="mon"
      w = 1
    ...
    @days.each do |day|
      ...
      day[:wday_class] = @@wday_specs[w][:class]
      day[:holiday] = holiday?(day[:year], day[:month], day[:day], w) || nil
      hclass = day[:holiday] ? ' holiday' : ''
      day[:class] += ' ' + @@wday_specs[w][:class] + hclass
      if @@wday_specs[w][:class] == "sun" ||  @@wday_specs[w][:class] == "sat"
        day[:header_class] = @@wday_specs[w][:class]
      else
        if hclass.blank?
          day[:header_class] = "weekday"
        else
          day[:header_class] = "holiday"
        end
      end
      day[:td_class] = "#{day[:header_class]} #{day[:range]}"
      w = (w == 6) ? 0 : w + 1
    end
  end
  ...
  def week_days(start_idx)
    header_days = []
    7.times{|i|
      header_days << @days[start_idx + i]
    }
    return header_days
  end
